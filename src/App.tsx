...
export default function App() {
  ...
  const handleRemoveFromBase = (id: string) => { removeLeadFromBase(id); showToast('Removido', 'info'); };
  ...
  const getTabContent = () => {
    ...
    if (activeTab === 'base') return <LeadsBaseTable leads={baseLeads as any} onStatusChange={(id, s) => updateLeadStatus(id, s)} onRemoveFromBase={handleRemoveFromBase} />;
    if (activeTab === 'disparo') return <LeadsDispatchWhatsApp leads={baseLeads as any} onClose={() => setActiveTab('base')} onStatusChange={(id, s) => updateLeadStatus(id, s)} onRemoveFromBase={handleRemoveFromBase} />;
    ...
    if (activeTab === 'scraper') return <GoogleMapsScraper onImportComplete={(_l) => { showToast('Importado!', 'success'); refetch(); setActiveTab('buscar'); }} />;
    return null;
  }
